...
- **Vanilla HTML/CSS/JS** — `index.html` + `styles.css` 구성. ES 모듈 import 없이 한 `<script>` 안에서 전부 동작
- **빌드 툴 없음** — npm, webpack, vite, TypeScript 모두 미사용. 이 결정은 의도적이다 — 핸드폰에서 GitHub Pages로 5초만에 반영되는 사이클이 가장 중요하기 때문
- **데이터와 스타일 분리** — JS 로직은 `index.html` 안, 스타일은 `styles.css`, 튜닝 가능한 값은 `data/*.json`
...
```
ikkiuchi-modular/
├── index.html          ← 진입점. HTML + 모든 게임 로직(JS) + admin console (~3,100줄)
├── styles.css          ← 모든 스타일 (~2,365줄)
...
### index.html 내부 구조 (JS 섹션)

```
<script>
// ============================================================
// 1. DATA LOADER              데이터 로딩 및 전역 변수
// 2. GAME STATE & UTILITIES   game/state 객체 + 공통 함수
// 3. CLASS SELECT SCREEN      클래스 선택 화면
// 4. MAP SCREEN               맵 화면 (5스테이지 노드)
// 5. COMBAT SCREEN & RENDERING  전투 화면 UI 렌더링
// 6. AI DECISION SYSTEM       적 AI 의사결정
// 7. TURN RESOLUTION ENGINE   턴 해결 로직 (위치/상성/데미지)
// 8. GAME OUTCOME HANDLERS    승패 처리
// 9. SHOP SCREEN              상점
// 10. FORGE SCREEN            대장간
// 11. DOJO SCREEN             도장 (기예 레벨업)
// 12. ADMIN CONSOLE           데이터 편집 + GitHub commit
// 13. INITIALIZATION          앱 시작점
// ============================================================
</script>
```
...
- **빌드 시스템 도입 안 함** — npm, bundler, TS 추가 제안 거부. HTML + CSS + JSON의 단순함이 핵심 가치
- **JS 코드 파일 분리 금지** — `index.html`의 `<script>` 안에 모든 로직 유지. CSS는 `styles.css`로 분리됨 (가독성), 데이터는 `data/*.json` (튜닝 용이성). **JS는 한 파일 원칙 유지** (핸드폰 편집 편의)
...
| 스타일 수정 | `styles.css` — 색상 변수는 `:root` 섹션 |
| 새 화면 추가 | `index.html`의 `<section class="screen">` (HTML) + `showScreen()` 로직 (JS) |
